...
export function importCategoryDraft(raw: unknown): CategoryDraft {
    if (
        !isObject(raw) ||
    ...
    const questions: QuestionDraft[] = raw.questions.map((q, qIndex) => {
        if (
            !isObject(q) ||
            typeof q.text !== "string" ||
            typeof q.answer !== "string"
        ) {
            throw new Error(`Invalid question at row ${qIndex}`)
        }
        ...
        if (q.questionMedia !== undefined) {
            if (!isMediaDraft(q.questionMedia)) {
                throw new Error(`Invalid questionMedia at row ${qIndex}`)
            }
            questionMedia = q.questionMedia
        }
        ...
        if (q.answerMedia !== undefined) {
            if (!isMediaDraft(q.answerMedia)) {
                throw new Error(`Invalid answerMedia at row ${qIndex}`)
            }
            answerMedia = q.answerMedia
        }
